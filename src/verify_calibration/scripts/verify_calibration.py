...
from verify_calibration.srv import *
...
class ARTrackAndPick:
    def __init__(self):
        ...
        # 定义ROS服务/vision_demo_pickup的服务端，服务回调函数为call_pick_place()
        self.pick_place_srv = rospy.Service('/vision_demo_pickup', CallPickPlaceDemo, self.call_pick_place)

        # 初始化规划组
        self.xarm = MoveGroupCommander(GROUP_NAME_ARM)
        self.xarm.set_max_velocity_scaling_factor(0.01)
        self.xarm.set_max_acceleration_scaling_factor(0.01)
        
        # self.gripper = MoveGroupCommander(GROUP_NAME_GRIPPER)

        # 机械臂先回到初始位置
        self.xarm.set_named_target('Handeye_Calibration')
        self.xarm.go()
        ...
        rospy.loginfo("Pick and Place demo is ready. You can call the service /vision_demo_pickup to test... ")
        rospy.spin()
    ...
    def pick_and_place(self,target_id,target_pose,place_pose):
        ...
        while result != MoveItErrorCodes.SUCCESS and n_attempts < max_pick_attempts:
            ...
            rospy.loginfo("dir(self.xarm): " +  dir(self.xarm))
